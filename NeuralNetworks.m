...
try
   load(fullfile(path,file),'net');
    ...
    msgbox('N�o foi possivel carregar a rede! error:' + err.message,'Erro','error');
    return;
end
...
try
   save(fullfile(path,file),'net');
    ...
    msgbox('N�o foi possivel guardar a rede! error:' + err.message,'Erro','error');
    return;
end
...
if(isempty(handles.myData.testImages))
    msgbox('N�o existem dados para treino!. Por favor, Selecione os dados!','Erro','error');
    return;
end
...
%Display precision
handles.textPrecision.String = strcat('Precis�o Global: ',num2str(handles.myData.globalPrecision),'%');
handles.textPrecisionTest.String = strcat('Precis�o Teste: ',num2str(handles.myData.testPrecision),'%');
plotperf(tr);
...
if(isequal(path,0))
 ...
 if ~exist(fullfile(path,'square'),'dir')
    msgbox('Pasta de quadrados (\squares) nao existe! Os dados n�o importados','Erro','error');
    return;
 end
 
 if ~exist(fullfile(path,'star'),'dir')
    msgbox('Pasta de estrelas (\stars) n�o foi encontrada! Os dados n�o foram importados','Erro','error');
    return;
 end
 
  if ~exist(fullfile(path,'circle'),'dir')
    msgbox('Pasta de circulos (\circles) n�o foi encontrada! Os dados n�o foram importados','Erro','error');
    return;
  end
 
  if ~exist(fullfile(path,'triangle'),'dir')
    msgbox('Pasta de triangulos(\triangles) n�o foi encontrada! Os dados n�o foram importados','Erro','error');
    return;
  end
...
if(isempty(handles.myData.runInput))
    %'Ficheiros para treino n�o existem. Por favor, importe os dados!'
...
precisionData = {'Precis�o Global','Precis�o dos Testes', ' '; handles.myData.globalPrecision, handles.myData.testPrecision, ''};
...
[x,y] = size(table);
if x <= 0
    msgbox('N�o existem dados na tabela para que seja necess�rio exportar!','Erro','error');
    return;
end
...
try
    ...
    msgbox('Erro ao guarda! verifique se o ficheiro esta aberto ou o excel est� instalado!', 'Erro ao guardar','error');
end
